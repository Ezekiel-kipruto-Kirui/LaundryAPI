...
/**
 * RESOLVE DATE SAFELY
 * Handles both 'created_at' and 'date' fields, including nulls.
 * Returns "YYYY-MM-DD" string or null.
 */
const resolveDate = (item: any): string | null => {
  // Priority 1: created_at (Standard ISO timestamp)
  if (item.created_at) {
  ...
  // Priority 2: date field (Expense records, or explicit overrides)
  if (item.date) {
...
export default function PerformanceReport() {
  ...
  const calculateShopMetrics = useCallback((
    orders: LaundryOrder[],
    filteredExpenses: LaundryExpense[],
    ...
    const shopExpenses = filteredExpenses
      .filter(e => e.shop === shopName)
  ...
  const data = useMemo<ProcessedData | null>(() => {
    ...
    if (fromDate) {
      ...
      const years: number[] = [];
      [...hotelOrders, ...laundryOrders, ...hotelExpenses, ...laundryExpenses].forEach(item => {
        const iso = resolveDate(item);
        if (iso) years.push(parseInt(iso.split('-')[0], 10));
      });
      if (years.length > 0) derivedYear = Math.max(...years);
    }
    ...
    // --- 3. DATE CHECK HELPER ---
    const isDateInRange = (item: any): boolean => {
      const itemISO = resolveDate(item);
      if (!itemISO) return false;
      return itemISO >= effectiveStartDate && itemISO <= effectiveEndDate;
    };

    // --- 4. FILTER DATA ARRAYS ONCE ---
    const filteredHotelOrders = hotelOrders.filter(o => isDateInRange(o));
    const filteredLaundryOrders = laundryOrders.filter(o => isDateInRange(o));
    const filteredHotelExpenses = hotelExpenses.filter(e => isDateInRange(e));
    const filteredLaundryExpenses = laundryExpenses.filter(e => isDateInRange(e));

    // --- 5. HOTEL CALCULATIONS ---
    const hotelRevenue = filteredHotelOrders.reduce((sum, order) => sum + parseMoney(order.total_amount), 0);
    ...
    // --- 8. SHOP METRICS ---
    const shopA = calculateShopMetrics(filteredLaundryOrders, filteredLaundryExpenses, 'Shop A');
    const shopB = calculateShopMetrics(filteredLaundryOrders, filteredLaundryExpenses, 'Shop B');
    ...
    // --- 11. MONTHLY TRENDS (FIXED TO USE FILTERED DATA) ---
    ...
    const addToMonthly = (items: any[], valKey: string, arr: number[]) => {
      items.forEach(item => {
        const itemISO = resolveDate(item);

        // STRICT FILTER: Only add if the item is in the derived year AND matches the current range logic
        // We check startswith(derivedYear) to respect year, and rely on the array being pre-filtered 
        // or just check boundaries manually if the array passed in is RAW.
        // However, to ensure STRICT compliance with range filters, we must check boundaries.

        if (itemISO && itemISO >= effectiveStartDate && itemISO <= effectiveEndDate) {
          const monthIndex = parseInt(itemISO.split('-')[1], 10) - 1;
    ...
    // USE FILTERED ARRAYS for aggregation to ensure charts match summary cards
    addToMonthly(filteredHotelOrders, 'total_amount', monthlyHotelRevenue);
    addToMonthly(filteredLaundryOrders, 'total_price', monthlyLaundryRevenue);
    addToMonthly(filteredHotelExpenses, 'amount', monthlyHotelExpenses);
    addToMonthly(filteredLaundryExpenses, 'amount', monthlyLaundryExpenses);
    ...
    return {
      ...
      totalExpenses: laundryExpensesTotal + hotelExpensesTotal,
      shopA,
      shopB,
